 ...
 */
export const fetchBranchDiff: GitLabFetchFunction<
  FetchBranchParams,
  FetchBranchResult
> = async (logger, { gitLabBaseUrl, headers, targetBranch, sourceBranch }) => {
...
> = async (logger, { gitLabBaseUrl, headers, changesOldPaths }) => {
  logger.info("Fetching old file versions", {
    fileCount: changesOldPaths.length,
    files: changesOldPaths,
  });

  const oldFilesRequestUrls = changesOldPaths.map(
    (path: string) =>
      new URL(
        `${gitLabBaseUrl}/repository/files/${encodeURIComponent(path)}/raw`,
      ),
  );
...
function extractCompleteJson(jsonText: string): string {
  ...
  if (braceCount > 0) {
    ...
    const result =
      (inString ? jsonText + '"' : jsonText) + "}".repeat(braceCount);
    return result;
  }
...
async function findExistingBotComment(
  ...
  const notesUrl = new URL(
    `${params.gitLabBaseUrl}/merge_requests/${params.mergeRequestIid}/notes`,
  );
  ...
  try {
    ...
    const botNotes = notes.filter(
      (note) => !note.system && note.body?.includes("🤖"), // Our bot comments include this emoji
    );
...
async function updateAIComment(
  ...
  const updateUrl = new URL(
    `${params.gitLabBaseUrl}/merge_requests/${params.mergeRequestIid}/notes/${params.noteId}`,
  );
 ...
 */
export const postAIComment: GitLabFetchFunction<
  PostAICommentParams,
  PostAICommentResult
> = async (
  logger,
 ...
 */
export const postInlineComments: GitLabFetchFunction<
  PostInlineCommentsParams,
  PostInlineCommentsResult
> = async (
  logger,
  {
    gitLabBaseUrl,
  ...
  const discussionsUrl = new URL(
    `${gitLabBaseUrl}/merge_requests/${mergeRequestIid}/discussions`,
  );
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    try {
      ...
      logger.error(
        `Failed to post inline comment ${i + 1}/${comments.length}`,
        error,
        {
          file: comment.file,
          line: comment.line,
    ...
    if (!response.ok) {
      ...
      logger.error(
        `GitLab API error for inline comment ${i + 1}/${comments.length}`,
        undefined,
        {
          file: comment.file,
          line: comment.line,
      ...
      logger.debug(
        `Successfully posted inline comment ${i + 1}/${comments.length}`,
        {
          file: comment.file,
          line: comment.line,
        },
...
function tryRegexParsing(
  ...
  try {
    ...
    let summary = "";
    const summaryMatch1 = aiResponse.match(
      /"summary"\s*:\s*"((?:[^"\\]|\\.)*)"/s,
    );
    ...
    const inlineCommentsSection = aiResponse.match(
      /"inline_comments"\s*:\s*\[([\s\S]*?)(?:\]|$)/s,
    );
    ...
    if (inlineCommentsSection) {
      ...
      const pattern1 =
        /\{\s*"file"\s*:\s*"([^"]+)"\s*,\s*"line"\s*:\s*(\d+)\s*,\s*"comment"\s*:\s*"((?:[^"\\]|\\.)*)"\s*\}/gs;
...
export function parseStructuredResponse(
  ...
  try {
    ...
    logger.error(
      "Both parsing methods failed, using full response as summary",
      error,
      {
 ...
 */
export const approveMergeRequest: GitLabFetchFunction<
...
> = async (
  ...
  const approveUrl = new URL(
    `${gitLabBaseUrl}/merge_requests/${mergeRequestIid}/approve`,
  );
