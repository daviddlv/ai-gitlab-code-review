 ...
 */
function shouldApproveMR (logger: Logger, aiResponse: string): boolean {
  ...
  logger.info('MR approval decision', {
    ...
    hasPositiveKeywords,
    hasNegativeKeywords,
    foundPositiveKeywords: hasPositiveKeywords ? positiveKeywords.filter(k => lowerResponse.includes(k)) : []
  })
...
export async function postAIReview (
  ...
  if (webhookResult instanceof Error) {
    logger.error('Webhook result is an error, skipping AI review', webhookResult)
    return
  }
  ...
  const { gitLabBaseUrl, mergeRequestIid, provider, modelName, messages, baseSha, headSha, startSha } = webhookResult
  ...
  try {
    ...
    if (result instanceof Error) {
      ...
      const aiComment = await postAIComment(logger, {
        gitLabBaseUrl,
        mergeRequestIid,
...
async function handleStructuredMode (
  ...
  const { answer, gitLabBaseUrl, mergeRequestIid, baseSha, headSha, startSha } = params
  ...
  if (summary && summary.trim()) {
    ...
    const commentPayload = buildCommentPayload(buildAnswer(summary), webhookBody.object_kind)
    const aiComment = await postAIComment(logger, {
      gitLabBaseUrl,
      mergeRequestIid,
      headers: fastify.gitLabFetchHeaders
    ...
    logger.warn('Structured response has no content, posting full response as global comment')
